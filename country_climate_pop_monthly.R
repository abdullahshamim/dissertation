...
# attach Washington DC climate data

data_dc <- data_states %>% 
  filter(statefp == '11') %>% 
  mutate(month = as.numeric(month)) %>% 
  select(-iso3, -state_id) %>% 
  mutate(countyfp = '001') %>% 
  select(-humidity)

data_counties <- data_counties %>% 
  filter(year %in% 1950:2022) %>% 
  select(-hddays,-cddays) %>% 
  mutate(month = as.numeric(month)) %>% 
  bind_rows(data_dc) %>% 
  arrange(statefp) %>% 
  relocate(precip, .after = month)


# prepare country data for comparison with county data
data <- data %>% 
  mutate(country_name = countrycode::countrycode(iso3, 'iso3c', 'country.name'), .before = everything()) %>% 
  mutate(country_name = case_match(iso3, "KSV" ~ "Kosovo", .default = country_name)) %>% 
  mutate(month = as.numeric(month))
