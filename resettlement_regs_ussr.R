...
formulas$baseline <- as.formula(paste("placements ~ ", paste(baseline_vars, collapse = " + ")))
formulas$a1 <- as.formula(paste("placements ~ ", paste(c("nearby_arrivals_idw_norm", a1_vars), collapse = " + ")))
formulas$a2 <- as.formula(paste("placements ~ ", paste(c("nearby_arrivals_idw_norm", a2_vars), collapse = " + ")))
formulas$a <- as.formula(paste("placements ~ ", paste(c("nearby_arrivals_idw_norm", a_vars), collapse = " + ")))
formulas$b <- as.formula(paste("placements ~ ", paste(c(baseline_vars, a_vars), collapse = " + ")))
formulas$c <- as.formula(paste("placements ~ ", paste(c(baseline_vars, a_vars, c_vars), collapse = " + ")))
formulas$d <- as.formula(paste("placements ~ ", paste(c(baseline_vars, a_vars, c_vars, d_vars), collapse = " + ")))
# formulas$e <- as.formula(paste("placements ~ ", paste(c(baseline_vars, a_vars, c_vars, d_vars, e_vars), collapse = " + ")))
formulas$f <- as.formula(paste("placements ~ ", paste(c(baseline_vars, a_vars, d_vars, c_vars, e_vars, f_vars), collapse = " + ")))
...
highlight_vars = list(
  ...
  "rowcol, orangered!15, se" = c("Same-Origin Resettlements in Nearby CZs", "Lagged Same-Origin CZ Population", "Lagged Same-Origin Pop. in Nearby CZs")
)
...
etable(reg1, digits = "r3", digits.stats = "r4",
       group = groups, order = varorder, dict = dict, 
       highlight = highlight_vars, view = T)

# etable(reg1, digits = "r3", digits.stats = "r4", order = order, dict = dict, tex = T, view = T, file = "Output/Tables/regs/latex_tables/ResettlementDriversRegs/yrfe.tex")
